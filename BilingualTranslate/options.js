...
async function checkOptModelStatus() {
  ...
  const prog = $("optModelProgressContainer");
  ...
  if (typeof Translator === "undefined") {
    ...
    desc.textContent = "Your browser uses instant cloud fallback. All features work seamlessly.";
  ...
  try {
    ...
    if (availability === "available" || availability === "readily") {
      ...
      badge.textContent = "Cloud Fallback Ready";
      badge.className = "chip bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300";
      desc.textContent = `Offline model not available for ${src}→${tgt}. Instant cloud fallback handles all translations.`;
      btn.classList.add("hidden");
    }
  } catch {
    badge.textContent = "Cloud Fallback Ready";
    badge.className = "chip bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300";
    desc.textContent = "Instant cloud fallback active. All features are fully operational.";
    btn.classList.add("hidden");
  }
}

async function startOptModelDownload() {
  const src = $("src").value;
  const tgt = $("tgt").value;
  const btn = $("optDownloadModelBtn");
  const prog = $("optModelProgressContainer");
  const bar = $("optModelProgressBar");
  const badge = $("optModelBadge");
  ...
  try {
    const translator = await Translator.create({
      ...
      monitor(m) {
        m.addEventListener("downloadprogress", (e) => {
          ...
          btn.textContent = `Downloading (${loadedPct}%)…`;
        });
      },
    });
    translator?.destroy?.();
    ...
    badge.textContent = "Ready ✓";
    badge.className = "chip bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300";
    $("optModelDesc").textContent = "Language model installed! All features are ready on-device.";
    btn.classList.add("hidden");
...
$("src").addEventListener("change", checkOptModelStatus);
$("tgt").addEventListener("change", checkOptModelStatus);
$("optDownloadModelBtn")?.addEventListener("click", startOptModelDownload);
